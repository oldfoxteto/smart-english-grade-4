...
WITH en_a2 AS (
  ...
  FROM levels lv
  JOIN languages l ON l.id = lv.language_id
  WHERE l.code = 'en' AND lv.cefr_level = 'A2'
), en_tracks_a2 AS (
  SELECT t.id, t.code
  FROM tracks t
  JOIN languages l ON l.id = t.language_id
  WHERE l.code = 'en'
    AND t.code IN ('daily-life-basics', 'travel-core', 'work-communication')
)
INSERT INTO units (track_id, level_id, title, sort_order)
SELECT et.id, ea.level_id,
  CASE et.code
    WHEN 'daily-life-basics' THEN 'Daily Dialogues A2'
    WHEN 'travel-core' THEN 'Travel Problem Solving A2'
    WHEN 'work-communication' THEN 'Work Communication A2'
    ELSE 'A2 Unit'
  END,
  2
FROM en_tracks_a2 et
CROSS JOIN en_a2 ea
WHERE NOT EXISTS (
  SELECT 1 FROM units u WHERE u.track_id = et.id AND u.level_id = ea.level_id
);
...
WITH el_a1 AS (
  ...
  FROM levels lv
  JOIN languages l ON l.id = lv.language_id
  WHERE l.code = 'el' AND lv.cefr_level = 'A1'
), el_tracks AS (
  SELECT t.id, t.code
  FROM tracks t
  JOIN languages l ON l.id = t.language_id
  WHERE l.code = 'el'
    AND t.code IN ('daily-life-basics', 'travel-core')
)
INSERT INTO units (track_id, level_id, title, sort_order)
SELECT et.id, ea.level_id,
  CASE et.code
    WHEN 'daily-life-basics' THEN 'Greek Alphabet and Greetings'
    WHEN 'travel-core' THEN 'Greek Travel Essentials'
    ELSE 'Greek Starter Unit'
  END,
  1
FROM el_tracks et
CROSS JOIN el_a1 ea
...
INSERT INTO lessons (unit_id, lesson_type, title, est_minutes, sort_order)
SELECT u.id, 'speaking', 'Speaking Drill', 6, 2
FROM units u
WHERE NOT EXISTS (
  SELECT 1 FROM lessons l WHERE l.unit_id = u.id AND l.sort_order = 2
);
